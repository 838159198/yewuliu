 ...
 */
class SummaryWeek extends CActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			array('title, mid, createtime, endtime, createdatetime, enddatetime,statdatetime', 'required'),
			array('title', 'length', 'max'=>30),
			array('mid, createtime, endtime', 'length', 'max'=>10),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, title, mid, createtime, endtime, createdatetime, enddatetime', 'safe', 'on'=>'search'),
		);
	}
